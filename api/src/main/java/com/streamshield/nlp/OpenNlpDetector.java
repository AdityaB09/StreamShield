...
public class OpenNlpDetector {
  ...
  public void init() {
    if (!enabled) return;
    try {
      person = new NameFinderME(new TokenNameFinderModel(new FileInputStream("/app/models/en-ner-person.bin")));
      org    = new NameFinderME(new TokenNameFinderModel(new FileInputStream("/app/models/en-ner-organization.bin")));
      loc    = new NameFinderME(new TokenNameFinderModel(new FileInputStream("/app/models/en-ner-location.bin")));
  ...
  public List<EntityHit> detect(String text) {
    ...
    add(out, text, tokens, offsets, loc.find(tokens), "LOC");
    return out;
  }

  private void add(List<EntityHit> out, String text, String[] tokens, int[] offsets, Span[] spans, String type) {
    for (Span s : spans) {
      ...
      out.add(new EntityHit(type, span, start, end, 0.75, "OPENNLP"));
    }
  }
}
